    ...
    public static class PersonalPerkPatches
    {
        ...
        public static void InitializePatches(Harmony harmony)
        {
            ...
            foreach (var m in candidates)
            {
                try 
                { 
                    // Skip abstract, virtual interface methods that can't be patched
                    if (m.IsAbstract || (m.IsVirtual && m.DeclaringType.IsInterface))
                    {
                        SelectClassTraitsMain.LogDebug($"Skipped abstract/interface method: {m.DeclaringType?.FullName}.{m.Name}");
                        continue;
                    }
                    
                    harmony.Patch(m, postfix: postfix); 
                    ok++; 
                    SelectClassTraitsMain.LogDebug($"Patched: {m.DeclaringType?.FullName}.{m.Name}"); 
                }
                catch (Exception e) 
                { 
                    fail++; 
                    SelectClassTraitsMain.LogDebug($"Failed to patch {m.DeclaringType?.FullName}.{m.Name}: {e.Message}"); 
                }
            }
